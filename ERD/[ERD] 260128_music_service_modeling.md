...
* **핵심 목적**: 가수, 음반, 노래 간의 복잡한 관계를 분석하고, 데이터 중복을 최소화하며 확장성을 고려한 DB 스키마를 설계합니다.
...
* **장르의 다각화**: 장르를 단순히 텍스트로 넣지 않고 별도 테이블로 구성하여 노래, 앨범, 가수와 각각 연결함으로써 정교한 데이터 필터링이 가능하게 했습니다.

### 3. 고난도 관계 처리: 다대다(M:N)와 자기 참조 🧠

가장 고민이 깊었던 부분으로, 복잡한 현실 세계의 관계를 논리적인 테이블 구조로 풀어냈습니다.
...
* **다대다 관계의 늪**: 처음에는 테이블 간 직접 연결을 시도했으나, 데이터 중복 문제를 발견하고 매핑 테이블의 필요성을 체감하며 논리적 설계를 수정했습니다.
...
네, 이전에 작성하신 공공데이터 API TIL 양식()을 바탕으로, 이번 **음악 데이터베이스 모델링** 과정을 체계적으로 정리한 마크다운(Markdown) 초안입니다.

각 제목에 이모티콘을 추가하고, 실습 코드 부분을 제외하여 핵심 설계 로직과 고민 과정에 집중할 수 있도록 구성했습니다.
